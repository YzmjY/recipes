...
struct Date {
	...
	uint16_t dayOfWeek() const { return chrono::weekday(days_).c_encoding(); }
...
private:
	struct GroupIterator {
		...
		GroupIterator& operator++() {
			cur = next_cur;
			if (cur != last) {
				next_cur = ranges::find_if_not(
				    ranges::next(cur), last,
		...
		friend bool operator==(const GroupIterator& iter,
		                       std::default_sentinel_t) {
			return iter.cur == iter.last;
		}

		// C++20 defaulted comparison operators
		friend bool operator==(const GroupIterator& lhs,
		                       const GroupIterator& rhs) = default;
...
struct concat_view {
	...
	struct ConcatIterator {
		using difference_type = std::ptrdiff_t;
		using value_type =
		    std::common_type_t<ranges::range_reference_t<Rngs>...>;

		ConcatIterator() = default;
		ConcatIterator(RNGs* rngs)
		    : rngs(rngs) {
		...
		using reference =
		    std::common_reference_t<ranges::range_reference_t<Rngs>...>;
		reference operator*() const {
			return std::visit(
			    [](auto&& iter) -> reference { return *iter.iterator; }, its);
		}

		friend bool operator==(const ConcatIterator& iter,
		                       std::default_sentinel_t) {
		...
		friend bool operator==(const ConcatIterator& lhs,
		                       const ConcatIterator& rhs) = default;
	...
	private:
		...
		struct IteratorWithIndex : std::integral_constant<size_t, N> {
			...
			friend bool operator==(const IteratorWithIndex& lhs,
			                       const IteratorWithIndex& rhs) = default;
		};

		template <size_t... Is>
		static constexpr auto
		    iteratorVariantGenerator(std::index_sequence<Is...>)
		        -> std::variant<
		            IteratorWithIndex<Is, std::tuple_element_t<Is, RNGs>>...>;

		decltype(iteratorVariantGenerator(
		    std::make_index_sequence<nRngs>{})) its;
		RNGs* rngs{};
	};
...
struct repeat_n_view : ranges::view_interface<repeat_n_view<Value>> {
	...
	repeat_n_view(size_t n, Value value)
	    : n(n)
...
private:
	struct RepeatIterator {
		...
		friend bool operator==(const RepeatIterator& iter,
		                       std::default_sentinel_t) {
			return iter.n == 0;
		}

		friend bool operator==(const RepeatIterator& lhs,
		                       const RepeatIterator& rhs) = default;
...
struct _Join : views::__adaptor::_RangeAdaptorClosure {
	...
	constexpr auto operator()(Rng&& rng) const {
		...
		using JoinResult =
		    std::conditional_t<ranges::range<RangeValue> &&
		                           !ranges::view<RangeValue>, // is container?
		                       RangeValue,
		                       std::vector<ranges::range_value_t<RangeValue>>>;
		JoinResult res;
		for (auto&& r : rng) {
			res.insert(ranges::end(res), ranges::begin(r), ranges::end(r));
		}
		return res;
	}
};
...
private:
	struct ChunkIterator {
		...
		friend bool operator==(const ChunkIterator& iter,
		                       std::default_sentinel_t) {
			return iter.cur == iter.last;
		}

		friend bool operator==(const ChunkIterator& lhs,
...
private:
	struct InterleaveInterator {
		...
		InterleaveInterator& operator++() {
			if (((++n) %= its.size()) == 0) {
				ranges::for_each(
				    its, [](auto&& iter) { iter = ranges::next(iter); });
			}
			return *this;
		}
		...
		friend bool operator==(const InterleaveInterator& iter,
		                       std::default_sentinel_t) {
			if (iter.n != 0)
				return false;
			auto ends = *iter.rngs | views::transform(ranges::end);
			return ranges::mismatch(iter.its, ends, std::equal_to<>{}).in1 ==
			       iter.its.end();
		}

		friend bool operator==(const InterleaveInterator& lhs,
		                       const InterleaveInterator& rhs) = default;
...
auto layout_months() {
	return views::transform([](/*range<Date>*/ auto&& month) {
		...
		return concat(views::single(month_title(*ranges::begin(month))),
